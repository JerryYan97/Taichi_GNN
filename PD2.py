...
class PD_Simulation:
    ...
    # @ti.kernel
    def set_Force(self, ind, magid):
        self.exf_ind = ind
        self.mag_ind = magid
        x = 0.3*magid * ti.sin(3.1415926/30.0*ind)
        y = 0.3*magid * ti.cos(3.1415926/30.0*ind)
    ...
    def build_sn(self):
        for vert_idx in range(self.NV):  # number of vertices
            ...
            self.Sn[Sn_idx1] = pos_i[0]+self.dt*vel_i[0]+(self.dt*self.dt)*self.ex_force[0][0]  # x-direction;
            self.Sn[Sn_idx2] = pos_i[1]+self.dt*vel_i[1]+(self.dt*self.dt)*(self.gravity[1]+self.ex_force[0][1])  # y-direction;
    ...
    def dataoneframe(self, input_p, input_v):
        self.copy(input_p, self.pos) # set from input
