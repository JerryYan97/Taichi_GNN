...
# @ti.func  # self.curr_rel_pos, self.initial_rel_pos
# def calcA_pq(num_neigh):
#     sum = ti.Matrix([[0, 0], [0, 0]])
#     for i in range(num_neigh):
#         sum += ti.outer_product(pi[i], qi[i].transpose()) # ti.outer_product(a, b)
#     return sum
...
# @ti.func
# def calcR2(A_pq, A_qq):
#     R, S = ti.polar_decompose(A_pq.dot(A_qq.inv()), real)
#     return R
...
def output_all(dispd, dispn, gradE, frame, istest):
    frame = str(frame).zfill(2)
    if istest == 0:
        outname = "Outputs/output"+str(exf_ind)+"_"+str(mag_ind)+"_"+frame+".txt"
    else:
        outname = "Outputs_T/output" + str(exf_ind) + "_" + str(mag_ind) + "_" + frame + ".txt"
    if not os.path.exists(outname):   
        file = open(outname, 'w+')
        file.close()
    ele_count = dim+dim+dim*dim+dim+dim  # pd pos + pn pos + local transform + residual + force
    out = np.ones([n_particles, ele_count], dtype = float)
    mesh.enable_connectivity()

    for i in range(n_particles):
        out[i,0] = dispd[i,0] # pd pos
        out[i,1] = dispd[i,1] 
        out[i,2] = dispn[i,0] # pn pos
        out[i,3] = dispn[i,1]
        adjv = mesh.get_vertex_adjacent_vertices(i)
        adjv = np.append(adjv, i)
        adjv = np.sort(adjv)
        ...
        out[i,8] = gradE[i*2]   # output_deformation_gradient
        out[i,9] = gradE[i*2+1]

        # tfx = m[i] * v[i][0] / dt
        # tfy = m[i] * v[i][1] / dt
        tempf = np.array([m[i] * v[i][0] / dt, m[i] * v[i][1] / dt])
        out[i,10] = npex_f[0] + tempf[0]  # output_deformation_gradient
        out[i,11] = npex_f[1] + tempf[1]
    np.savetxt(outname, out)
...
if __name__ == "__main__":
    TESTMODE = 0
    if TESTMODE is 1:
        ShowTestData("TestResult")
    elif TESTMODE is 0:
        pd = PD_Simulation(int(1), dim)
        ...
        # extreme_test()
        initial_com = calcCenterOfMass(np.arange(n_particles))
        initial_rel_pos = x.to_numpy() - initial_com
        print("init center of mass: ", initial_com)

        for forceid in range(1):
            generate_exforce()
            compute_exforce(exf_ind, mag_ind)
            print(exf_ind, " -- ", mag_ind, "  ", ex_force[0][0], " -- ", ex_force[0][1])
            npex_f[0] = ex_force[0][0]
            npex_f[1] = ex_force[0][1]
            print("npex_f: ", npex_f[0], " ", npex_f[1])
            # npex_f = (ex_force[0]).to_numpy() # update the exf force
            x.from_numpy(mesh.vertices.astype(np.float32))
            v.fill(0)
            pd.set_Force(exf_ind, mag_ind)
            ...
            for f in range(50):
                ...
                pos_result2, pd_pos_new, pd_vel_new = pd.dataoneframe(inputxn, inputvn)
                # print(pos_result2.to_numpy())
                ...
                while True:
                    ...
                    data_sol.from_numpy(solve_linear_system(data_mat.to_numpy(), data_rhs.to_numpy(), n_particles * dim,
                                                            np.array([i for i in range(11)]), zero.to_numpy(), False, 0,
                                                            cnt[None]))
                ...
                compute_v()
                # print("after x: \n", x)
                output_all(pos_result2.to_numpy(), deltap.to_numpy(), gradE.to_numpy(), f, istest)
