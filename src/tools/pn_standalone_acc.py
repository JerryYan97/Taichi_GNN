# It failed to make it on GPU, because I am not able to implement makePD in Taichi func/kernel. :(
import sys, os, time
...
def set_ring_force_3D():
    for i in range(n_particles):
        # ex_force[i] = get_ring_force_field(4, 10.0, ti_center, x[i], 0.0, 3)
...
@ti.kernel
def fill_left_mat(row_array: ti.ext_arr(), col_array: ti.ext_arr(), val_array: ti.ext_arr(),
                  tmp_triples: ti.ext_arr(), fixed_array: ti.ext_arr(), delete_idx: ti.ext_arr(), i_cnt: ti.int32):
    # NOTE:
    # 200000 is the max number of threads that Taichi kernel can call in one time for ti.gpu.
    # However, it is common that our sys_dim is much larger than this number.
    # CPU's max calling workload is same.
    if i_cnt > 100000:
        thread_workloads = i_cnt // 100000
        for i in range(100000):
            # [start_idx, end_idx)
            start_idx = i * thread_workloads
            end_idx = start_idx + thread_workloads
            if i == 100000 - 1:
                end_idx = i_cnt - 1
            cur_thread_workloads = end_idx - start_idx
            for j in range(cur_thread_workloads):
                cur_idx = start_idx + j
                row_idx = int(tmp_triples[0, cur_idx])
                col_idx = int(tmp_triples[1, cur_idx])
                if fixed_array[row_idx] != 1 and fixed_array[col_idx] != 1:
                    row_array[cur_idx] = row_idx
                    col_array[cur_idx] = col_idx
                    val_array[cur_idx] = tmp_triples[2, cur_idx]
                    delete_idx[cur_idx] = 1
    else:
        for i in range(i_cnt):
            row_idx = int(tmp_triples[0, i])
            col_idx = int(tmp_triples[1, i])
            if fixed_array[row_idx] != 1 and fixed_array[col_idx] != 1:
                row_array[i] = row_idx
                col_array[i] = col_idx
                val_array[i] = tmp_triples[2, i]
                delete_idx[i] = 1


def check_sym(a, tol=1e-8):
    return np.all(np.abs(a - a.T) < tol)


# Time consuming parts: Cholesky factorization and sparse matrix construction.
def my_solve_linear_system(data_mat, data_rhs, sys_dim, i_dirichlet, i_cnt):
    # Fill dirichlet boundary and compute right
    fixed = np.full(sys_dim, 0, dtype=np.int)  # 1.0 -- fixed, 0.0 -- !fixed.
    right = np.copy(data_rhs)
    dirichlet_expanded = np.full(i_dirichlet.shape[0] * dim, 0, dtype=np.int)
    # If dirichlet is too long (>1000), then it may needs to be written in kernel.
    for i in range(i_dirichlet.shape[0]):
        for d in range(dim):
            fixed[i_dirichlet[i] * dim + d] = 1
            right[i_dirichlet[i] * dim + d] = 0.0
            dirichlet_expanded[i * dim + d] = i_dirichlet[i] * dim + d
    # Compute left
    # project pd  -- NOTE: PD projection is in the compute Hessian.
    left_mat_row = np.full(i_cnt, 0, dtype=np.int)
    left_mat_col = np.full(i_cnt, 0, dtype=np.int)
    left_mat_val = np.full(i_cnt, 0.0, dtype=np.float64)
    delete_idx = np.full(i_cnt, 0, dtype=np.int)  # 1 -- keep, 0 -- delete.
    fill_left_mat(left_mat_row, left_mat_col, left_mat_val, data_mat, fixed, delete_idx, i_cnt)  # 10444278
    left_mat_row = left_mat_row[delete_idx.astype(bool)]
    left_mat_col = left_mat_col[delete_idx.astype(bool)]
    left_mat_val = left_mat_val[delete_idx.astype(bool)]

    # Note: Construct csc/csr matrix is time-consuming: it takes about 0.265s, if we don't delete redundant elements.
    dirichlet_mat = sparse.csc_matrix((np.ones(i_dirichlet.shape[0] * dim, dtype=np.int),
                                       (dirichlet_expanded, dirichlet_expanded)),
                                      shape=(sys_dim, sys_dim),
                                      dtype=np.float64)
    left_mat = sparse.csc_matrix((left_mat_val, (left_mat_row, left_mat_col)),
                                 shape=(sys_dim, sys_dim),
                                 dtype=np.float64)

    left_mat += dirichlet_mat

    # Record: Arm_17698v_72161t -- factor: 1.1469006538391113s, solve: 0.018208026885986328
    factor = cholesky(left_mat)
    sol = factor(right)
    return sol
...
if __name__ == "__main__":
    ...
    data_rhs = np.zeros(shape=(dim * n_particles,), dtype=np.float64)
    ...
    for f in range(60):
        set_ring_force_3D()
        print("==================== Frame: ", f, " ====================")
        # Whole frame time Record -- 1e-4 * dt -- ARM -- frame 2
        # Old: 21.57075047492981
        # New: 22.80263090133667
        ...
        while True:
            ...
            # my_solve_linear_system(data_mat, data_rhs, n_particles * dim, np.array(dirichlet), cnt[None])
            # time_start = time.time()
            if dim == 2:
                ...
                # Solve system Record -- ARM:
                # Old: 1.3967013359069824
                # New: 1.5114164352416992s
                solve_linear_sys_time_start = time.time()
                # data_sol = solve_linear_system3(data_mat, data_rhs, n_particles * dim,
                #                                 np.array(dirichlet), zero.to_numpy(),
                #                                 False, 0, cnt[None])
                data_sol = my_solve_linear_system(data_mat, data_rhs, n_particles * dim, np.array(dirichlet), cnt[None])
                solve_linear_sys_time_end = time.time()
                print("solve linear system time:", solve_linear_sys_time_end - solve_linear_sys_time_start)
            # time_end = time.time()
        ...
        print("whole frame time:", frame_time_end - frame_time_start)
