...
class PNSimulation:
    def __init__(self, objfilenum, _dim):
        ...
        self.vertices = ti.var(ti.i32)
        self.boundary_points = ti.var(ti.i32)
        self.boundary_edges = ti.var(ti.i32)
        ...
        self.cnt = ti.var(dt=ti.i32, shape=())
    ...
    def output_all(self, pd_dis, pn_dis, grad_E, frame, T):
        ...
        for i in range(self.n_particles):
            ...
            temp_f = np.array([self.m[i] * self.x_xtilde[i][0], self.m[i] * self.x_xtilde[i][1]])
            ...
            # out[i, 12] = delta_pos[i, 0]
            # out[i, 13] = delta_pos[i, 1]
        np.savetxt(out_name, out)
...
if __name__ == "__main__":
    ...
    for forceid in range(1):
        ...
        for f in range(50):
            ...
            pn.data_rhs.fill(0)
            pn.data_sol.fill(0)
            # print("shape of pd_ pos_new:", pd_pos_new.shape())
            ...
            while True:
                ...
                # print("sol: ", pn.data_sol.to_numpy())
                # pn.print_sol()
            ...
            pn.compute_v()
            # pn.print_sol()
            # print("xPrev: ", pn.xPrev.to_numpy())
            # print("vel: ", pn.v.to_numpy())
            pn.compute_x_xtilde()
            # print("after x: \n", x)
            pn.output_all(pos_result2.to_numpy(), pn.del_p.to_numpy(), pn.gradE.to_numpy(), f, istest)
            # pn.output_p(pos_result2.to_numpy(), pn.del_p.to_numpy(), f, istest)
            total_time += time.time()
