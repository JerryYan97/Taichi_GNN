...
writer = SummaryWriter('../runs/GCN_1007_double')  # default `log_dir` is "runs" - we'll be more specific here
###################################################
...
def Sim_train():
    ...
    for epoch in range(args.epochs):
        ...
        for data in train_loader:  # Iterate in batches over the training dataset.
            col = int(data.edge_index.shape[1] / 2)
            edge_ind = torch.cat((data.edge_index[:, :col], data.edge_index[:, :col]), 0)
            output = model(data.x.float().to(device),
                           edge_ind.to(device),
                           int(data.num_graphs / bs),
